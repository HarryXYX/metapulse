...
export default defineConfig(async ({ mode }) => {
    ...
    // Setup proxy to the SwotData backend service.
    // 支持两种环境变量：VITE_BACKEND_URL（新）和 REACT_APP_PROXY_TARGET（兼容）
    const backendUrl = process.env.VITE_BACKEND_URL || process.env.REACT_APP_PROXY_TARGET || 'http://localhost:8080';
    const frontendProxy = {
        target: backendUrl,
        changeOrigin: true,
        secure: false,
    ...
    return {
        ...
        plugins: [
            ...
            codecovVitePlugin({
                ...
                bundleName: 'metapulse-react-web',
        ...
        // },
        envPrefix: ['REACT_APP_', 'VITE_'], // 支持两种环境变量前缀
        build: {
            outDir: 'dist',
            target: 'esnext',
            minify: 'esbuild',
        ...
        test: {
            ...
            coverage: {
                ...
                include: ['src/**/*.ts'],
                reportsDirectory: '../build/coverage-reports/metapulse-web-react/',
                exclude: [],
            },
        },
        resolve: {
