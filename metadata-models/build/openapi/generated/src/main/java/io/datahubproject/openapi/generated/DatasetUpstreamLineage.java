package io.datahubproject.openapi.generated;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import io.datahubproject.openapi.generated.DatasetFieldMapping;
import io.swagger.v3.oas.annotations.media.Schema;
import java.util.ArrayList;
import java.util.List;
import org.springframework.validation.annotation.Validated;
import javax.validation.Valid;
import com.fasterxml.jackson.annotation.JsonInclude;
import javax.validation.constraints.*;

import lombok.Builder;
import lombok.Getter;
import lombok.extern.jackson.Jacksonized;
import com.fasterxml.jackson.annotation.JsonTypeInfo;
/**
 * Fine Grained upstream lineage for fields in a dataset
 */
@Schema(description = "Fine Grained upstream lineage for fields in a dataset")
@Validated
@javax.annotation.Generated(value = "io.swagger.codegen.v3.generators.java.SpringCodegen", date = "2025-11-04T23:27:37.210949+08:00[Asia/Shanghai]")
@Jacksonized @Builder(toBuilder = true)
@JsonTypeInfo(use = JsonTypeInfo.Id.NAME, include = JsonTypeInfo.As.EXISTING_PROPERTY, property = "__type")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class DatasetUpstreamLineage  implements OneOfDatasetSnapshotAspectsItems, OneOfEnvelopedAspectValue, OneOfGenericAspectValue {

  @Getter(value = lombok.AccessLevel.NONE)
  @JsonProperty(value = "__type", defaultValue = "DatasetUpstreamLineage") @Builder.Default
  private String __type = "DatasetUpstreamLineage";

  /**
   * Name of this subclass in SimpleClassName format
   * @return __type
   **/
  @Schema(required = true, description = "Name of this subclass in SimpleClassName format", allowableValues = {"DatasetUpstreamLineage"},
  defaultValue = "DatasetUpstreamLineage")
  @NotNull
  public String get__type() {
    return __type;
  }

  @JsonProperty("fieldMappings") @Builder.Default
  @Valid
  private List<DatasetFieldMapping> fieldMappings = new ArrayList<>();

  public DatasetUpstreamLineage fieldMappings(List<DatasetFieldMapping> fieldMappings) {
    this.fieldMappings = fieldMappings;
    return this;
  }

  public DatasetUpstreamLineage addFieldMappingsItem(DatasetFieldMapping fieldMappingsItem) {
    this.fieldMappings.add(fieldMappingsItem);
    return this;
  }

  /**
   * Upstream to downstream field level lineage mappings
   * @return fieldMappings
   **/
  @Schema(required = true, description = "Upstream to downstream field level lineage mappings")
      @NotNull
    @Valid
    public List<DatasetFieldMapping> getFieldMappings() {
    return fieldMappings;
  }

  public void setFieldMappings(List<DatasetFieldMapping> fieldMappings) {
    this.fieldMappings = fieldMappings;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    DatasetUpstreamLineage datasetUpstreamLineage = (DatasetUpstreamLineage) o;
    return Objects.equals(this.fieldMappings, datasetUpstreamLineage.fieldMappings);
  }

  @Override
  public int hashCode() {
    return Objects.hash(fieldMappings);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class DatasetUpstreamLineage {\n");
    
    sb.append("    fieldMappings: ").append(toIndentedString(fieldMappings)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}